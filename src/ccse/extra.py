...
@register
class CrushAdapter:
    """Crush ~/.config/crush/crush.json (providers, credentials, model
    selection) + ~/.local/share/crush/providers.json (Catwalk model catalog).

    The catalog is regenerated by crush from Catwalk on launch, so external
    edits to providers.json don't stick. The durable selection lives in
    crush.json: the model is declared on providers.<id>.models and chosen via
    the top-level models.large = {provider, model}, which overrides the
    catalog's default_large_model_id. providers.json is still mirrored
    best-effort (older crush builds and diff visibility)."""
    id = "crush"
    name = "Crush"
    primary = "crush.model"
    ...
    def _selected_model(self):
        ...
        if isinstance(sel, dict):
            large = sel.get("large")
            if isinstance(large, dict) and large.get("model"):
                return large["model"]
        return None
    ...
    def slots(self):
        ...
        out = [Slot(key="crush.model",
                    label=f"models.large (provider {pid})",
                    current=self._selected_model()
                    or (entry.get("default_large_model_id") if entry else None))]
    ...
    def apply(self, assignments, dry=False):
        ...
        if "model" in relevant:
            ...
            # crush.json: declare the model on the provider and select it as
            # the default large model — both survive the Catwalk refresh that
            # regenerates providers.json on crush launch
            pmodels = p.get("models")
            if not isinstance(pmodels, list):
                pmodels = p["models"] = []
            if not any(isinstance(x, dict) and x.get("id") == model
                       for x in pmodels):
                pmodels.append({"id": model, "name": model})
                diffs.append(f"  providers.{pid}.models[{model!r}]: (created)")
                ep_dirty = True
            sel = d.get("models")
            if not isinstance(sel, dict):
                sel = d["models"] = {}
            old = sel.get("large")
            if not (isinstance(old, dict) and old.get("provider") == pid
                    and old.get("model") == model):
                diffs.append(f"  models.large: {old!r} -> "
                             f"{{'provider': {pid!r}, 'model': {model!r}}}")
                sel["large"] = {"provider": pid, "model": model}
                ep_dirty = True
            # providers.json mirror: default_large_model_id + model entry
            # (overwritten by crush's Catwalk sync, but keeps older builds
            # and interactive diffs correct until then)
            cat = config.load_json(self._providers_path())
            entry = next((q for q in cat
                          if isinstance(q, dict) and q.get("id") == pid),
                         None) if isinstance(cat, list) else None
            if entry is None:
                diffs.append(f"  (skip: provider {pid!r} not in providers.json)")
            else:
                if entry.get("default_large_model_id") != model:
                    diffs.append(f"  providers.json[{pid}].default_large_model_id: "
                                 f"{entry.get('default_large_model_id')!r} -> "
                                 f"{model!r}")
                    entry["default_large_model_id"] = model
                    cat_dirty = True
                cmodels = entry.get("models")
                if not isinstance(cmodels, list):
                    cmodels = entry["models"] = []
                if not any(isinstance(x, dict) and x.get("id") == model
                           for x in cmodels):
                    cmodels.append({"id": model, "name": model})
                    diffs.append(f"  providers.json[{pid}].models[{model!r}]: "
                                 "(created)")
                    cat_dirty = True
        ...
        if not dry:
            if ep_dirty:
                config.keep_mode_write_json(self.path, d)
            if cat_dirty and isinstance(cat, list):
                config.keep_mode_write_json(self._providers_path(), cat)
        return diffs


# ─────────────────── Ante: settings.json + provider env ──────────────────

@register
class AnteAdapter:
    """Ante ~/.ante/settings.json: model + provider selection.

    settings.json carries `model` (bare model id, no router prefix) and
    `provider` (a catalog id, see `ante catalog`). Custom gateways use the
    fixed `openai-compatible` catalog provider, whose base_url/api_key come
    from the OPENAI_COMPATIBLE_BASE_URL / OPENAI_COMPATIBLE_API_KEY env vars
    (persisted in the shell rc). When a gateway endpoint is switched and
    settings still have no provider, it is set to `openai-compatible` —
    otherwise ante keeps its default provider and ignores the vars."""
    id = "ante"
    name = "Ante"
    primary = "ante.model"
    path = HOME / ".ante" / "settings.json"
    model_raw = True

    _ENV = {"base_url": "OPENAI_COMPATIBLE_BASE_URL",
    ...
    def slots(self):
        if not self.available:
            return []
        d = config.load_json(self.path) or {}
        return [Slot(key="ante.model", label="model", current=d.get("model")),
                Slot(key="ante.provider", label="provider",
                     current=d.get("provider")),
                Slot(key="ante.base_url",
                     label="env OPENAI_COMPATIBLE_BASE_URL",
                     current=self._env_current(self._ENV["base_url"]),
                     kind=KIND_BASE_URL),
                Slot(key="ante.api_key",
                     label="env OPENAI_COMPATIBLE_API_KEY",
                     current=self._env_current(self._ENV["api_key"]),
                     kind=KIND_API_KEY)]

    def apply(self, assignments, dry=False):
        relevant = {k[len("ante."):]: v for k, v in assignments.items()
                    if k.startswith("ante.")}
        if not relevant or not self.available:
            return []
        d = config.load_json(self.path) or {}
        diffs: list[str] = []
        ...
        if "model" in relevant and d.get("model") != relevant["model"]:
            ...
            d["model"] = relevant["model"]
            dirty = True
        if "provider" in relevant:
            cur = d.get("provider")
            if cur != relevant["provider"]:
                diffs.append(f"  provider: {cur!r} -> {relevant['provider']!r}")
                d["provider"] = relevant["provider"]
                dirty = True
        elif ("base_url" in relevant or "api_key" in relevant) \
                and not d.get("provider"):
            diffs.append("  provider: None -> 'openai-compatible' "
                         "(gateway env vars feed this catalog provider)")
            d["provider"] = "openai-compatible"
            dirty = True
        for key, var in self._ENV.items():
            if key not in relevant:
                continue
            old = self._env_current(var)
            if old == relevant[key]:
                continue
